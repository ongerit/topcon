...
(function($, vsc){
    ...
    function initialize($component){
        var $checkPolicyForm = $component.find('.nyl-form-change-password__form--check-policy');
        var $changePasswordForm = $component.find('.nyl-form-change-password__form');
        var reasonParam = vsc.getParameter('reason');
        vsc.insertReason(reasonParam, $changePasswordForm.find('#reason'));

        initCheckPolicyForm($checkPolicyForm);
        initForm($changePasswordForm);

        $component.find('a.nyl-form-change-password__backtoform').click(vsc.showFullForm);
    }

    function changeUSorOther(event){
        var $target = $(event.target);
        var $section = $target.closest('.nyl-form-change-password__fieldset--personal-info');

        if ($target.val().toLowerCase() === 'other'){
            $section.find('.nyl-form-change-password__fieldset--other-country').show();
            $section.find('.nyl-form-change-password__fieldset--united-states').hide();
        }
        else {
            $section.find('.nyl-form-change-password__fieldset--other-country').hide();
            $section.find('.nyl-form-change-password__fieldset--united-states').show();
        }
    }
    ...
    function initForm($form){
        ...
        var $personalInfoSection = $form.find('.nyl-form-change-password__fieldset--personal-info');

        var $dobFields = $form.find('.nyl-form-change-password__input--dob-month, .nyl-form-change-password__input--dob-day, .nyl-form-change-password__input--dob-year');
        initSegmentedFields($dobFields);

        var $ssnFields = $form.find('.nyl-form-change-password__input--ssn-1, .nyl-form-change-password__input--ssn-2, .nyl-form-change-password__input--ssn-3');
        initSegmentedFields($ssnFields);

        var $zipFields = $personalInfoSection.find('.nyl-form-change-password__input--zip, .nyl-form-change-password__input--zip-ext');
        initSegmentedFields($zipFields);

        var $phoneFields = $personalInfoSection.find('.nyl-form-change-password__input--phone-area-code, .nyl-form-change-password__input--phone-first-3, .nyl-form-change-password__input--phone-last-4');
    ...
    function _postFormData(form) {
        ...
        var postData = {
            component: 'static-forms-change-password',
            options: formData
        };
        ...
        $.ajax({
            ...
            success: function(result) {
                if (result.status === 'OK') {
                    _buildModal('static-forms-change-password', 'success', false, $form);
                ...
                _buildModal('static-forms-change-password', 'error', errorCode, $form);
            },
            error: function(errorResult) {
                console.warn('Failed to post Static Forms Change Password data to service');
            }
        });
    }
    ...
    $(document).ready(function(){
        ...
        var $component = $('.nyl-form-change-password');
