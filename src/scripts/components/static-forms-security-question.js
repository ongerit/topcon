...
(function($, vsc) {
    ...
    function initialize($component){
        var $checkPolicyForm = $component.find('.nyl-form-security-question__form--check-policy');
        var $securityQuestionForm = $component.find('.nyl-form-security-question__form');
        var reasonParam = vsc.getParameter('reason');
        vsc.insertReason(reasonParam, $securityQuestionForm.find('#reason'));

        initCheckPolicyForm($checkPolicyForm);
        initForm($securityQuestionForm);

        $component.find('a.nyl-form-security-question__backtoform').click(vsc.showFullForm);
    }

    function changeUSorOther(event){
        var $target = $(event.target);
        var $section = $target.closest('.nyl-form-security-question__fieldset--personal-info');

        if ($target.val().toLowerCase() === 'other'){
            $section.find('.nyl-form-security-question__fieldset--other-country').show();
            $section.find('.nyl-form-security-question__fieldset--united-states').hide();
        }
        else {
            $section.find('.nyl-form-security-question__fieldset--other-country').hide();
            $section.find('.nyl-form-security-question__fieldset--united-states').show();
        }
    }
    ...
    function initForm($form){
        ...
        var $personalInfoSection = $form.find('.nyl-form-security-question__fieldset--personal-info');

        var $dobFields = $form.find('.nyl-form-security-question__input--dob-month, .nyl-form-security-question__input--dob-day, .nyl-form-security-question__input--dob-year');
        initSegmentedFields($dobFields);

        var $ssnFields = $form.find('.nyl-form-security-question__input--ssn-1, .nyl-form-security-question__input--ssn-2, .nyl-form-security-question__input--ssn-3');
        initSegmentedFields($ssnFields);

        var $zipFields = $personalInfoSection.find('.nyl-form-security-question__input--zip, .nyl-form-security-question__input--zip-ext');
        initSegmentedFields($zipFields);

        var $phoneFields = $personalInfoSection.find('.nyl-form-security-question__input--phone-area-code, .nyl-form-security-question__input--phone-first-3, .nyl-form-security-question__input--phone-last-4');
    ...
    function _postFormData(form) {
        ...
        var postData = {
            component: 'static-forms-security-question',
            options: formData
        };
        ...
        $.ajax({
            ...
            success: function(result) {
                if (result.status === 'OK') {
                    _buildModal('static-forms-security-question', 'success', false, $form);
                ...
                _buildModal('static-forms-security-question', 'error', errorCode, $form);
            },
            error: function(errorResult) {
                console.warn('Failed to post Static Forms Security Question data to service');
            }
        });
    }
    ...
    $(document).ready(function(){
        ...
        var $component = $('.nyl-form-security-question');
