...
(function($, vsc){
    ...
    function initialize($component){
        var $checkPolicyForm = $component.find('.nyl-form-registration__form--check-policy');
        var $registrationForm = $component.find('.nyl-form-registration__form');
        var reasonParam = vsc.getParameter('reason');
        vsc.insertReason(reasonParam, $registrationForm.find('#reason'));

        initCheckPolicyForm($checkPolicyForm);
        initForm($registrationForm);

        $component.find('a.nyl-form-registration__backtoform').click(vsc.showFullForm);
    }

    function changeUSorOther(event){
        var $target = $(event.target);
        var $section = $target.closest('.nyl-form-registration__fieldset--personal-info');

        if ($target.val().toLowerCase() === 'other'){
            $section.find('.nyl-form-registration__fieldset--other-country').show();
            $section.find('.nyl-form-registration__fieldset--united-states').hide();
        }
        else {
            $section.find('.nyl-form-registration__fieldset--other-country').hide();
            $section.find('.nyl-form-registration__fieldset--united-states').show();
        }
    }
    ...
    function initForm($form){
        ...
        var $personalInfoSection = $form.find('.nyl-form-registration__fieldset--personal-info');

        var $dobFields = $form.find('.nyl-form-registration__input--dob-month, .nyl-form-registration__input--dob-day, .nyl-form-registration__input--dob-year');
        initSegmentedFields($dobFields);

        var $ssnFields = $form.find('.nyl-form-registration__input--ssn-1, .nyl-form-registration__input--ssn-2, .nyl-form-registration__input--ssn-3');
        initSegmentedFields($ssnFields);

        var $zipFields = $personalInfoSection.find('.nyl-form-registration__input--zip, .nyl-form-registration__input--zip-ext');
        initSegmentedFields($zipFields);

        var $phoneFields = $personalInfoSection.find('.nyl-form-registration__input--phone-area-code, .nyl-form-registration__input--phone-first-3, .nyl-form-registration__input--phone-last-4');
    ...
    function _postFormData(form) {
        ...
        var postData = {
            component: 'static-forms-registration',
            options: formData
        };
        ...
        $.ajax({
            ...
            success: function(result) {
                if (result.status === 'OK') {
                    _buildModal('static-forms-registration', 'success', false, $form);
                ...
                _buildModal('static-forms-registration', 'error', errorCode, $form);
            },
            error: function(errorResult) {
                console.warn('Failed to post Static Forms Registration data to service');
            }
        });
    }
    ...
    $(document).ready(function(){
        var $component = $('.nyl-form-registration');
